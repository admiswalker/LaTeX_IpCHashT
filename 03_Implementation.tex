...
軽いテストを行うには，limitSize を 5000000 として \$ make, \$ ./exe\_bench を実行すると，./tmpBench ディレクトリが生成され，10 分程で ベンチマーク結果が保存される．
グラフのスケールが合っていない場合は，plots.py を調整する必要がある．

本投稿に示すベンチマークは，limitSize を 200000000，loopNum を 100 として，\$ make, \$ ./exe\_bench を実行すると，
tmpBench 内にベンチマーク結果の CSV ファイルが生成され，
次に \$ ./exe\_sProc を実行すると，CSV ファイルから中央値が計算され，グラフとして出力される．
各ベンチマークの CSV ファイルを生成するには 2 日程掛かる．
