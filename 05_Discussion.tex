...
一方で，
{\bf std::unordered\_map} や {\bf sstd::CHashT}，{\bf sstd::IpCHashT (uint8, maxLF50)}，{\bf google::dense\_hash\_map} は，
挿入可能な要素数が $2\times10^8$ となるように制御されているため，リハッシュは発生していない．
加えて，
{\bf sstd::CHashT} は，
テーブルサイズの増加に伴い，緩やかにメモリ使用量を増加させていることが分かる．
このため，図\ref{fig_bench_memory} において，メモリ使用量の変化が折れ線に見えるのは，
メモリ使用量を測定する際の解像度不足と考えられる．
\leavevmode \newline
...
\footnote{
  正確には，$2.0\times10^8$ より大きな $2^k-1\ (k=1,2,...)$ の内，最も小さい値として $2^k-1|_{k=28} = 268435455$ となる．
  実際に ska::flat\_hash\_map がリハッシュしている区間は $1.250\times10^8〜1.375\times10^8$ であり，
  load factor が50 \% となる要素数 $268435455/2=134217727.5$ を含んでいる．
}
に対する速度を示す．
まず，{\bf sstd::IpCHashT (uint8, maxLF50)} と {\bf ska::flat\_hash\_map} は load factor が 50 \% を超えた時点でリハッシュしている．
...
{\bf sstd::IpCHashT (uint8, maxLF100)} については，
...
sstd::IpCHashT (uint16, maxLF100) については，load factor の増加に伴い，空き要素の線形探査に時間が掛かるためである．
なお，他の条件の sstd::IpCHashT は，比較的 load factor の低い領域を使用するため，影響は軽微である．
{\bf std::unordered\_map} は，挿入とリハッシュの両方に時間が掛かっていることが伺える．
{\bf sstd::IpCHashT (uint8, maxLF50)}，{\bf sstd::IpCHashT (uint8, maxLF100)}，{\bf ska::flat\_hash\_map}，{\bf google::dense\_hash\_map} に
ついては，計算量の違いは軽微である．
...
{\bf sstd::IpCHashT (uint8, maxLF50)} は，その挿入アルゴリズムの複雑さの割に高速な挿入である．
