...
%  \vspace{-5mm}
  ...
  \caption{
    ...
    ska::flat\_hash\_map while it is hard to see, is plotted on the same place of sstd::IpCHashT (uint8, maxLF50).
  }
...
%    And the width of peaks is measurement interval since the ture peak is as narrow as one step of x axis.
    sstd::IpCHashT (uint8, maxLF100), while it is hard to see, is plotted on the same place of sstd::IpCHashT (uint8, maxLF50).
    ska::flat\_hash\_map is initializing its table size with $2.0\times10^8$ and the others are initializing their table size with $2.0\times10^8$ as a containable size.
    This means that the one with a rehashing peak is ska::flat\_hash\_map.
  }
...
\begin{figure}[h]
  \includegraphics[scale=0.24]{./fig_bench/TMP_insert_et_preAlloc_med.pdf}
  \caption{
    {\bf \color{red}TODO: replace this benchmark after calculating 100 sample.}
    Total time of insertion, which is a median value of 100 samples, using pre-allocated table.
    In this measurement, each hash table is pre-allocated with $2.0\times10^8$.
    A discontinuous value around $1.25\textasciitilde.375\times10^8$ represents rehashing of ska::flat\_hash\_map.
...
\begin{figure}[h]
  \includegraphics[scale=0.24]{./fig_bench/TMP_insert_et_med.pdf}
  \caption{
    {\bf \color{red}TODO: replace this benchmark after calculating 100 sample.} Total time of insertion, which is a median value of 100 samples, with rehashing table.
    Chaining type of hash tables like, sstd::CHashT and sstd::IpCHashTs (especially uint16-maxLF100 option), are slow down its insertion speed as the load factor increase.
  }
