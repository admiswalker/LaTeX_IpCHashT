...
ハッシュテーブルでは，key から配列サイズに収まるハッシュ値を生成して，格納する．
一般には，ハッシュ値とテーブルサイズが互いに素となり易いように，テーブルサイズを素数としする．
そして，ハッシュ値をテーブルサイズで除算した余りを配列インデックスとする．

ところで，現在最も高速なハッシュテーブルの実装の一つは，
google::dense\_hash\_map である．
このハッシュテーブルは，
スループットで 250 query/ms 程度の探査速度を持つ\footnote{詳細は Benchmarks の章を参照．}．
