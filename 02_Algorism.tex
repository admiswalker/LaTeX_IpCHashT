...
実用に耐えうる定数を探すだけであれば，
実験的に求められる．
これには，図\ref{fig_bench_LF}に示す sstd::IpCHashT<uint64,uint64> (as uint8 and maxLF100) の load factor が最大となる，
最小のパディングサイズを探せばよい．
...
\begin{figure}[h]
  ...
  \caption{
    Deletion case05.
    削除時の断片化を低減する処理．
    空き要素を線形走査し，詰められるなくなるまで，再帰的に実行する必要があり，実行コストは非常に高い．
  }
