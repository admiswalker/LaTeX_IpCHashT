...
\begin{figure} % 特に強い理由がない限り、[htbp]のような指定はしないでください。
\begin{lstlisting}[language=C++]
struct itr sstd::IpCHashT<T_key, T_val>::find(T_key key_in){
	uint64 hashVal = hashFunc( key_in );
	uint64 idx = hashVal & tableSize_minus1;
	
	if(! isHead( table[ idx ] ) ){ return itr ( keyIsNotFound, maxIdx ); }
	for(;;){
		if( table[ idx ].key == key_in ){ return itr ( keyIsFound, idx ); }
		if( table[ idx ].next == 0 ){ return itr ( keyIsNotFound, maxIdx ); }
		idx += table[ idx ].next;
	}
}
\end{lstlisting}
...
struct itr sstd::IpCHashT<T_key, T_val>::find(T_key key_in){
	...
	for(;;){
		if( table[ idx ].key == key_in ){
			if( isEmpty( table[ idx ] ) ){ return itr ( keyIsNotFound, maxIdx ); }
			return itr ( keyIsFound, idx );
		}
		if( table[ idx ].next == 0 ){ return itr ( keyIsNotFound, maxIdx ); }
		idx += table[ idx ].next;
	}
