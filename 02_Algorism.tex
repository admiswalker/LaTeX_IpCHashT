...
\caption{
  ...
  And isHead() function is also gurantee the element is not empty
  because of the head element is identified by the previous linked list is 0, while having the previous linked list maximum value will indicating the element is empty.
}
\label{alg_find_sm}
\end{figure}
