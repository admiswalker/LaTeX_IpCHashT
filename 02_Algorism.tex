...
%通常の open hashing による chaining であれば，
%新しく確保したメモリアドレスをポインタにより接続するため，
%挿入済みの要素がメモリ上の何処に存在するかを考慮する必要はなく，
%メモリ管理は malloc 関数に任せればよい．
%しかし今回のように，
%連続したメモリアドレス上に doubly linked list を構築する，
%closed hashing による chaining を実現するには，
%ある程度メモリを自前で管理する必要がある．
